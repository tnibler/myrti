...
openapi:
  ...
  cd ../web && pnpm openapi-zod-client ../server/openapi.json -o src/lib/apiclient.ts --export-schemas
...
check-openapi:
  #!/usr/bin/bash
  tmpfile=`mktemp`
  cd web && pnpm openapi-zod-client ../server/openapi.json -o $tmpfile --export-schemas && cd ..
