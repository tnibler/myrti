...
use super::db_entity::{DbAlbum, DbAlbumWithItemCount, DbAsset, DbInsertAlbum};
use super::util::datetime_to_db_repr;
use super::{db_entity::DbAlbumItem, schema};
use crate::db::DbConn;
use crate::model::{self, Album, AlbumId, AlbumItem, AlbumItemId, AlbumItemType, Asset, AssetId};
    ...
    let items = rows
        ...
        .map(|row| match (row.asset, row.album_item.text) {
            (Some(asset), None) => {
                ...
                let asset = super::asset::get_asset(conn, AssetId(asset.asset_id))?;
                Ok(model::AlbumItem {
...
pub fn get_assets_in_album(
    ...
    db_assets
        ...
        .map(|db_asset| {
            // TODO: extra query per row, not nice
            super::asset::get_asset(conn, AssetId(db_asset.asset_id))
        })
        .collect::<Result<Vec<_>>>()
}
