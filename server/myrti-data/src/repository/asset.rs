...
use super::{
    db_entity::{
        DbAsset, DbAssetPathOnDisk, DbAssetThumbnail, DbInsertAsset, to_db_asset_ty,
        to_db_timezone_info,
    },
    util::{datetime_to_db_repr, hash_u64_to_vec8, to_db_thumbnail_type},
};
use crate::db::DbConn;
use crate::model::{
    self, Asset, AssetFile, AssetId, AssetPathOnDisk, AssetRootDirId, AssetSpe, AssetThumbnail,
    AssetThumbnailId, AssetType, CreateAsset, CreateAssetSpe, FileId, Image, IsOriginalStreamable,
    MirrorCorrection, OriginalStreaming, RotationCorrection, ThumbnailType, TimestampInfo, Video,
};
use crate::repository::db_entity::{
...
pub fn create_asset(conn: &mut DbConn, create_asset: CreateAsset) -> Result<AssetId> {
    ...
    conn.immediate_transaction(|conn| {
        ...
        let insertable: DbInsertAsset = DbInsertAsset {
            ...
            is_hidden: i32::from(create_asset.base.is_hidden),
...
pub fn set_assets_hidden(conn: &mut DbConn, set_hidden: bool, asset_ids: &[AssetId]) -> Result<()> {
    ...
    diesel::update(Asset::table.filter(Asset::asset_id.eq_any(asset_ids.iter().map(|id| id.0))))
        .set(Asset::is_hidden.eq(i32::from(set_hidden)))
        .execute(conn)
