...
const PhotoStackId = z.string();
const TimelineItem = z.union([
  AssetWithSpe.and(z.object({ itemType: z.literal('asset') }).passthrough()),
  z
    .object({
      assets: z.array(AssetWithSpe),
      itemType: z.literal('photoStack'),
      selectionIndices: z.array(z.number()),
      stackId: PhotoStackId,
    })
    .passthrough(),
]);
const TimelineSegment = SegmentType.and(
  z
    .object({ items: z.array(TimelineItem), sortDate: z.string().datetime({ offset: true }) })
    .passthrough(),
);
